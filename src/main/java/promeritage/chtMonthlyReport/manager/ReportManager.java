...
//        new CustomBean("10/05", "四", "做很多事情1", null) };
    ...
    public static final String WORK_CONTENT_TEMPLATE = 
            "工作${workNum}:\n" + 
    ...
    public static String getReportFormatWorkContent(JobDto customBean) {
        ...
        return template.replace("${workNum}", workNum)
                .replace("${workContent}", customBean.getWorkContent())
                .replace("${percentage}", percentage)
                .replace("${endDate}", customBean.getEndDate());
    }
    
    public static Collection<JobDto> getBeanCollection(List<JobDto> jobDtoList) {
        
//        List<CustomBean> result = new ArrayList<CustomBean>(Arrays.asList(data));
        ...
        for(int i = result.size()-1; i >= 0; i--){
            String endDate = result.get(i).getEndDate();
